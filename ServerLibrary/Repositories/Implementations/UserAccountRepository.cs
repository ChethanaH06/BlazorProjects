...
namespace ServerLibrary.Repositories.Implementations
{
    public class UserAccountRepository(IOptions<JwtSection> config,AppDbContext appDbContext) : IUserAccount
    {
        public async Task<GeneralResponse> CreateAsync(Register user)
        {
            ...
            var checkAdminRole = await appDbContext.SystemRoles.FirstOrDefaultAsync(c => c.Name!.Equals(Constants.Admin));
        ...
        public async Task<LoginResponse> SignInAsync(Login user)
        {
            ...
            var findUser = await appDbContext.RefreshTokenInfos.FirstOrDefaultAsync(c => c.UserId == applicationUser.Id);
        ...
        private async Task<UserRole> FindUserRole(int  userId)=>await appDbContext.UserRoles.FirstAsync(x => x.UserId == userId);
        private async Task<SystemRole> FindRoleName(int roleId) => await appDbContext.SystemRoles.FirstAsync(x => x.Id == roleId);
        ...
        public async Task<LoginResponse> RefreshTokenAsync(RefreshToken token)
        {
            ...
            var findToken = await appDbContext.RefreshTokenInfos.FirstOrDefaultAsync(b => b.Token!.Equals(token.Token));
            ...
            var updateRefreshToken=await appDbContext.RefreshTokenInfos.FirstOrDefaultAsync(n=>n.UserId==user.Id);
        ...
        public async Task<List<ManageUser>> GetUsers()
        {
            ...
            foreach (var user in allUsers)
            {
                ...
                var roleName = allRoles.FirstOrDefault(k => k.Id == userRole!.RoleId);
        ...
        {
            var user=await appDbContext.ApplicationUsers.FirstOrDefaultAsync(v=>v.Id==int.Parse(profile.Id));
