...
namespace ServerLibrary.Repositories.Implementations
{
    public class VacationRepository(AppDbContext appDbContext):IGenericRepositoryInterface<Vacation>
    {
        ...
        public async Task<List<Vacation>> GetAll() => await appDbContext.Vacations.AsNoTracking().Include(i=>i.VacationType).ToListAsync();

        public async Task<Vacation> GetById(int id) => await appDbContext.Vacations.FirstOrDefaultAsync(c => c.EmployeeId == id);
        ...
        {
            var obj = await appDbContext.Vacations.FirstOrDefaultAsync(n => n.EmployeeId == item.EmployeeId);
